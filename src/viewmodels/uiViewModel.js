...
class UiViewModel {
        ...
        init(vttApi) {
                ...
                if (!this.vttApi) {
                        log.error(
                                '[UiViewModel] VTT_API not provided during init!'
                        );
                ...
                if (selectedId) {
                        ...
                        this.inspectorData = null;
                        dUiVM(
                                'No object initially selected, inspectorData is null.'
                        );
                }
        }
        ...
        requestObjectDelete(objectId, objectName = 'object') {
                ...
                if (success) {
                        dUiVM(
                                'Object %s successfully deleted via API.',
                                objectId
                        );
                        this.displayMessage(
                                `Object "${objectName}" deleted.`,
                                'success'
                        );
                } else {
        ...
        _handleModelChange(event) {
                ...
                case 'selectionChanged':
                        this.inspectorData = payload
                                ? this.vttApi.getObject(payload)
                                : null;
                        dUiVM(
                                'Selection changed. New inspectorData: %o',
                                this.inspectorData
                        );
                ...
                case 'allObjectsCleared':
                        this.inspectorData = null;
                        dUiVM(
                                'All objects cleared. InspectorData set to null.'
                        );
                        refreshInspector = true;
                        break;
                ...
                default:
                        dUiVM(
                                'Unhandled model change type in UiViewModel: %s',
                                type
                        );
                }
                ...
                if (
                        ...
                        dUiVM(
                                'Inspector needs refresh, calling _onInspectorDataChanged.'
                        );
                ...
                if (
                        ...
                        dUiVM(
                                'Board settings need refresh, calling _onBoardSettingsChanged.'
                        );
        ...
        applyInspectorChanges(objectId, inspectorSnapshot) {
                ...
                if (!this.vttApi) {
                        dUiVM(
                                'applyInspectorChanges error: VTT API not available.'
                        );
                        return;
                }
                try {
                        ...
                        if (!currentObject) {
                                dUiVM(
                                        'applyInspectorChanges error: Object %s not found.',
                                        objectId
                                );
                                this.displayMessage(
                        ...
                        if (inspectorSnapshot.appearance) {
                                ...
                                const currentAppearance =
                                        currentObject.appearance || {};
                                for (const key in inspectorSnapshot.appearance) {
                                        if (
                                                inspectorSnapshot.appearance.hasOwnProperty(
                                                        key
                                                ) &&
                                ...
                                if (
                                        Object.keys(updatePayload.appearance)
                                                .length === 0
                                ) {
                        ...
                        if (inspectorSnapshot.scripts) {
                                ...
                                for (const key in inspectorSnapshot.scripts) {
                                        if (
                                                inspectorSnapshot.scripts.hasOwnProperty(
                                                        key
                                                ) &&
                                                ...
                                                updatePayload.scripts[key] =
                                                        inspectorSnapshot.scripts[
                                                                key
                                                        ];
                                                dUiVM(
                                                        'Inspector change detected for scripts.%s',
                                                        key
                                                );
                                        }
                                }
                                if (
                                        Object.keys(updatePayload.scripts)
                                                .length === 0
                                ) {
                        ...
                        dUiVM(
                                'Final updatePayload for object %s: %o',
                                objectId,
                                updatePayload
                        );
                        if (Object.keys(updatePayload).length > 0) {
                                this.vttApi.updateObject(
                                        objectId,
                                        updatePayload
                                );
                                this.displayMessage(
                                        `Object ${objectId} updated.`,
                                        'success',
                                        1500
                                );
                                ...
                                dUiVM(
                                        'No changes detected for object %s. No API call made.',
                                        objectId
                                );
                        }
                } catch (error) {
                        log.error(
                                '[UiViewModel] Error applying inspector changes:',
                                error
                        );
                        dUiVM(
        ...
        handleDeleteSelectedObject(objectId) {
                ...
                if (result) {
                        dUiVM(
                                'Object %s deleted via handleDeleteSelectedObject.',
                                objectId
                        );
                        this.displayMessage(
                                `Object ${objectId} deleted.`,
                                'success',
                                1500
                        );
                } else {
        ...
        applyBoardSettings(newProps) {
                ...
                if (!this.vttApi) {
                        dUiVM(
                                'applyBoardSettings error: VTT API not available.'
                        );
                        return;
                }
        ...
        createObject(shape, props = {}) {
                dUiVM(
                        'createObject called with shape: %s, props: %o',
                        shape,
                        props
                );
                if (!this.vttApi) {
        ...
        setTableBackground(backgroundProps) {
                ...
                if (!this.vttApi) {
                        dUiVM(
                                'setTableBackground error: VTT API not available.'
                        );
                        return;
                }
        ...
        requestCreateObjectModal() {
                ...
                if (typeof this._onCreateObjectModalRequested === 'function') {
                        ...
                        dUiVM(
                                '_onCreateObjectModalRequested callback invoked.'
                        );
                } else {
                        log.warn(
                        ...
                        this.displayMessage(
                                'Cannot open create object dialog.',
                                'error'
                        );
                }
        }
        ...
        requestLoadMemoryState() {
                ...
                const availableStates =
                        sessionManagement.getAvailableMemoryStates();
                        ...
                        this._onShowSelectionModalRequested(
                                ...
                                (selectedIndex) => {
                                        ...
                                        if (
                                                ...
                                                const stateToLoad =
                                                        sessionManagement.getMemoryStateByIndex(
                                                                selectedIndex
                                                        );
                                                if (stateToLoad) {
                                                        dUiVM(
                                                        ...
                                                        sessionManagement.applyMemoryState(
                                                                stateToLoad
                                                        );
                                                ...
                                                dUiVM(
                                                        'Invalid selection from modal: %s',
                                                        selectedIndex
                                                );
                                                this.displayMessage(
                                                        'Invalid selection.',
                                                ...
                                                dUiVM(
                                                        'Load from memory cancelled by user.'
                                                );
                                                this.displayMessage(
                                                        'Load from memory cancelled.',
                                                        'info'
                                                );
                                        }
                                }
