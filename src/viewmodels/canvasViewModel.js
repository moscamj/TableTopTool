...
class CanvasViewModel {
        ...
        constructor(onDrawNeededCallback, displayMessageFn) {
                ...
                this.onDrawNeededCallback =
                        ...
                        (() => {
                                log.warn(
                                        'onDrawNeededCallback not set in ViewModel'
                                );
                                dCanvasVM(
                                        'onDrawNeededCallback not set, logging warning.'
                                );
                        });

                /** @type {function(text: string, type: string, duration?: number): void} Function to display user messages. */
                this.displayMessageFn =
                        displayMessageFn ||
                        ((msg, type) => {
                                log.warn(
                                        'displayMessageFn not set in ViewModel:',
                                        msg,
                                        type
                                );
                                dCanvasVM(
                                        'displayMessageFn not set, logging warning. Message: %s, Type: %s',
                                        msg,
                                        type
                                );
                        });
                ...
                this.viewModelBoardProperties = {
                        widthUser: 36,
                        heightUser: 24,
                        unitForDimensions: 'in', // User-facing dimensions
                        widthPx: 1000,
                        heightPx: 800, // Pixel dimensions for rendering (example defaults)
                        scaleRatio: 1,
                        unitForRatio: 'mm', // Scale interpretation
                };
        ...
        loadStateIntoViewModel(initialState) {
                dCanvasVM(
                        'loadStateIntoViewModel called with initialState: %o',
                        initialState
                );
                if (!initialState) {
                        log.error(
                                '[CanvasViewModel] loadStateIntoViewModel: initialState is undefined. Cannot populate ViewModel.'
                        );
                        dCanvasVM(
                                'loadStateIntoViewModel error: initialState is undefined.'
                        );
                ...
                if (
                        ...
                        Array.isArray(initialState.objects)
                ) {
                        initialState.objects.forEach((obj) =>
                                this.viewModelObjects.set(obj.id, { ...obj })
                        );
                }

                if (initialState.panZoomState) {
                        this.viewModelPanZoom = {
                ...
                this.viewModelSelectedObjectId =
                        initialState.selectedObjectId !== undefined
                                ? initialState.selectedObjectId
                                : null;

                if (initialState.boardProperties) {
                        this.viewModelBoardProperties = {
                                ...initialState.boardProperties,
                        };
                } else {
                        this.viewModelBoardProperties = {
                                widthUser: 36,
        ...
        addObjectToViewModel(objectData) {
                dCanvasVM(
                        'addObjectToViewModel called with objectData: %o',
                        objectData
                );
                if (!objectData || !objectData.id) {
                        log.error(
                                '[CanvasViewModel] addObjectToViewModel: Invalid objectData provided.',
                                objectData
                        );
                        dCanvasVM(
                                'addObjectToViewModel error: Invalid objectData. ID: %s',
                                objectData?.id
                        );
                        return;
                }
                ...
                dCanvasVM(
                        'Object %s added to viewModelObjects.',
                        objectData.id
                );
                if (objectData.appearance && objectData.appearance.imageUrl) {
                        dCanvasVM(
                                'Object %s has imageUrl, calling loadImage: %s',
                                objectData.id,
                                objectData.appearance.imageUrl
                        );
                        this.loadImage(
                                objectData.appearance.imageUrl,
                                objectData.appearance.imageUrl
        ...
        updateObjectInViewModel(objectId, updatedProps) {
                dCanvasVM(
                        'updateObjectInViewModel called for objectId: %s, updatedProps: %o',
                        objectId,
                        updatedProps
                );
                if (!this.viewModelObjects.has(objectId)) {
                        log.warn(
                                `[CanvasViewModel] updateObjectInViewModel: Object ID ${objectId} not found in ViewModel.`
                        );
                        dCanvasVM(
                                'updateObjectInViewModel warning: Object ID %s not found.',
                                objectId
                        );
                        return;
                }
                ...
                dCanvasVM(
                        'Object %s updated in viewModelObjects. New state: %o',
                        objectId,
                        finalUpdatedObject
                );
                ...
                if (
                        updatedProps.appearance &&
                        updatedProps.appearance.imageUrl
                ) {
                        dCanvasVM(
                                'Updated object %s has new imageUrl, calling loadImage: %s',
                                objectId,
                                updatedProps.appearance.imageUrl
                        );
                        this.loadImage(
                        ...
                        dCanvasVM(
                                'Updated object %s has imageUrl cleared, calling loadImage with null.',
                                objectId
                        );
        ...
        removeObjectFromViewModel(objectId) {
                dCanvasVM(
                        'removeObjectFromViewModel called for objectId: %s',
                        objectId
                );
                if (!this.viewModelObjects.has(objectId)) {
                        log.warn(
                                `[CanvasViewModel] removeObjectFromViewModel: Object ID ${objectId} not found in ViewModel.`
                        );
                        dCanvasVM(
                                'removeObjectFromViewModel warning: Object ID %s not found.',
                                objectId
                        );
                        return;
                }
        ...
        setPanZoomInViewModel(panZoomState) {
                dCanvasVM(
                        'setPanZoomInViewModel called with panZoomState: %o',
                        panZoomState
                );
                if (!panZoomState) {
                        log.error(
                                '[CanvasViewModel] setPanZoomInViewModel: panZoomState is undefined.'
                        );
                        dCanvasVM(
                                'setPanZoomInViewModel error: panZoomState is undefined.'
                        );
                        return;
                }
                this.viewModelPanZoom = { ...panZoomState };
                dCanvasVM(
                        'viewModelPanZoom updated: %o',
                        this.viewModelPanZoom
                );
        }
        ...
        locallyUpdatePanZoom(panX, panY, zoom) {
                dCanvasVM(
                        'locallyUpdatePanZoom called. panX: %s, panY: %s, zoom: %s',
                        panX,
                        panY,
                        zoom
                );
                ...
                if (changed) {
                        dCanvasVM(
                                'Local pan/zoom changed, calling onDrawNeededCallback.'
                        );
                        this.onDrawNeededCallback();
                } else {
        ...
        locallyUpdateObjectPosition(objectId, x, y) {
                dCanvasVM(
                        'locallyUpdateObjectPosition called for objectId: %s, x: %s, y: %s',
                        objectId,
                        x,
                        y
                );
                ...
                if (obj && (obj.x !== x || obj.y !== y)) {
                        ...
                        obj.y = y;
                        dCanvasVM(
                                'Local position of object %s updated to x: %s, y: %s. Calling onDrawNeededCallback.',
                                objectId,
                                x,
                                y
                        );
                        ...
                        dCanvasVM(
                                'Local position of object %s not changed.',
                                objectId
                        );
                } else {
                        dCanvasVM(
                                'locallyUpdateObjectPosition: Object %s not found.',
                                objectId
                        );
                }
        }
        ...
        setBackgroundInViewModel(backgroundState) {
                dCanvasVM(
                        'setBackgroundInViewModel called with backgroundState: %o',
                        backgroundState
                );
                if (!backgroundState) {
                        log.error(
                                '[CanvasViewModel] setBackgroundInViewModel: backgroundState is undefined.'
                        );
                        dCanvasVM(
                                'setBackgroundInViewModel error: backgroundState is undefined.'
                        );
                        return;
                }
                this.viewModelTableBackground = { ...backgroundState };
                dCanvasVM(
                        'viewModelTableBackground updated: %o',
                        this.viewModelTableBackground
                );
                if (backgroundState.type === 'image' && backgroundState.value) {
                        dCanvasVM(
                                'Background type is image, calling loadImage for URL: %s',
        ...
        setSelectedObjectInViewModel(selectedId) {
                dCanvasVM(
                        'setSelectedObjectInViewModel called with selectedId: %s',
                        selectedId
                );
                this.viewModelSelectedObjectId = selectedId;
                dCanvasVM(
                        'viewModelSelectedObjectId updated to: %s',
                        this.viewModelSelectedObjectId
                );
        }
        ...
        setBoardPropertiesInViewModel(boardProps) {
                dCanvasVM(
                        'setBoardPropertiesInViewModel called with boardProps: %o',
                        boardProps
                );
                if (!boardProps) {
                        log.error(
                                '[CanvasViewModel] setBoardPropertiesInViewModel: boardProps is undefined.'
                        );
                        dCanvasVM(
                                'setBoardPropertiesInViewModel error: boardProps is undefined.'
                        );
                        return;
                }
                this.viewModelBoardProperties = { ...boardProps };
                dCanvasVM(
                        'viewModelBoardProperties updated: %o',
                        this.viewModelBoardProperties
                );
        }
        ...
        loadImage(url, cacheKey) {
                ...
                dCanvasVM(
                        'loadImage called. URL: %s, CacheKey: %s',
                        url,
                        cacheKey
                );
                if (!url) {
                        if (this.loadedImages.has(cacheKey)) {
                                dCanvasVM(
                                        'URL is null/empty, removing image from cache for key: %s',
                                        cacheKey
                                );
                                this.loadedImages.delete(cacheKey);
                                if (this.onDrawNeededCallback) {
                                        dCanvasVM(
                                                'Calling onDrawNeededCallback after removing image from cache.'
                                        );
                                        this.onDrawNeededCallback();
                                }
                        } else {
                                dCanvasVM(
                                        'URL is null/empty, image not in cache for key: %s. No action.',
                                        cacheKey
                                );
                        }
                        return;
                ...
                if (existingImage && existingImage.status === 'loaded') {
                        dCanvasVM(
                                'Image %s already loaded and in cache. Status: %s. Triggering redraw.',
                                cacheKey,
                ...
                if (existingImage && existingImage.status === 'loading') {
                        dCanvasVM(
                                'Image %s is currently loading. Status: %s. No action.',
                                cacheKey,
                                existingImage.status
                        );
                        return;
                ...
                dCanvasVM(
                        'Loading image: %s. Setting status to "loading" for cacheKey: %s',
                        url,
                        cacheKey
                );
                this.loadedImages.set(cacheKey, {
                        img: null,
                        status: 'loading',
                });
                ...
                image.onload = () => {
                        dCanvasVM(
                                'Successfully loaded image: %s. CacheKey: %s. Status set to "loaded".',
                                url,
                                cacheKey
                        );
                        this.loadedImages.set(cacheKey, {
                        ...
                        if (this.onDrawNeededCallback) {
                                dCanvasVM(
                                        'Calling onDrawNeededCallback after image load success.'
                                );
                                this.onDrawNeededCallback();
                        }
                };
                image.onerror = (err) => {
                        log.error(
                                `[CanvasViewModel] Error loading image: ${url}`,
                                err
                        );
                        dCanvasVM(
                                'Error loading image: %s. CacheKey: %s. Status set to "error". Error: %o',
                                url,
                                cacheKey,
                                err
                        );
                        ...
                        if (url.startsWith('data:image/')) {
                                // This log is already specific enough
                        ...
                        if (this.onDrawNeededCallback) {
                                dCanvasVM(
                                        'Calling onDrawNeededCallback after image load error.'
                                );
                                this.onDrawNeededCallback();
                        }
        ...
        getObjectAtPosition(worldX, worldY) {
                ...
                if (
                        ...
                        log.error(
                                '[CanvasViewModel] getObjectAtPosition: Invalid worldX or worldY input',
                                { worldX, worldY }
                        );
                        dCanvasVM(
                                'getObjectAtPosition error: Invalid worldX or worldY. worldX: %s, worldY: %s',
                                worldX,
                                worldY
                        );
                        return null;
                ...
                if (!(this.viewModelObjects instanceof Map)) {
                        log.error(
                                '[CanvasViewModel] getObjectAtPosition: viewModelObjects is not a Map',
                                this.viewModelObjects
                        );
                        dCanvasVM(
                                'getObjectAtPosition error: viewModelObjects is not a Map. Value: %o',
                                this.viewModelObjects
                        );
                        return null;
                }

                const sortedObjects = Array.from(
                        this.viewModelObjects.values()
                ...
                for (const obj of sortedObjects) {
                        if (!obj || typeof obj !== 'object') {
                                log.warn(
                                        '[CanvasViewModel] getObjectAtPosition: Encountered invalid object',
                                        obj
                                );
                                dCanvasVM(
                                        'getObjectAtPosition warning: Encountered invalid object in sorted list: %o',
                                        obj
                                );
                                continue;
                        }
                        ...
                        if (
                                ...
                                log.warn(
                                        '[CanvasViewModel] getObjectAtPosition: Object with id',
                                        id,
                                        'has invalid coords/dims.',
                                        { x, y, width, height }
                                );
                                continue;
                        }
                        ...
                        if (shape === 'rectangle') {
                                if (rotation === 0) {
                                        ...
                                        const rotatedLocalY =
                                                ...
                                                localY * cosTheta;
                                        if (
                                                rotatedLocalX >= -width / 2 &&
                                                rotatedLocalX <= width / 2 &&
                                                rotatedLocalY >= -height / 2 &&
                                                rotatedLocalY <= height / 2
                                        ) {
                                                return id;
                                        }
                                ...
                                const distanceSq =
                                        (worldX - circleCenterX) ** 2 +
                                        (worldY - circleCenterY) ** 2;
