 ...
 */
function requestRedrawEvent() {
    document.dispatchEvent(new CustomEvent('stateChangedForRedraw', { bubbles: true, composed: true }));
}
...
export const VTT_API = {
    ...
    log: (message, contextObject) => {
        const prefix = contextObject ? `[Script Log - ${contextObject.name || contextObject.id}]` : '[Script Log - Global]';
