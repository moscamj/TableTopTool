...
export function VTT_API_INIT(config) {
        ...
        if (config && typeof config.showMessage === 'function') {
                ...
                log.error(
                        'VTT_API_INIT: showMessage callback not provided or invalid.'
                );
...
export const VTT_API = {
        ...
        getObject: (objectId, contextObject) => {
                dApi(
                        'getObject called for id: %s, context: %o',
                        objectId,
                        contextObject
                );
        ...
        updateObjectState: (objectId, newData, contextObject) => {
                ...
                if (obj) {
                        ...
                        dApi(
                                'Updating object %s with merged data: %o',
                                objectId,
                                updatedData
                        );
                        ...
                        dApi(
                                'updateObjectState completed for id: %s',
                                objectId
                        );
                } else {
                        log.warn(
                                `[VTT_API.updateObjectState] Object ${objectId} not found.`
                        );
                        dApi(
                                'updateObjectState failed: Object %s not found',
                                objectId
                        );
                }
        },
        ...
        log: (message, contextObject) => {
                ...
                dApi(
                        'VTT_API.log called. Prefix: %s, Message: %o',
                        prefix,
                        message
                ); // Added debug log
        ...
        createObject: (arg1, arg2) => {
                ...
                if (
                        ...
                        dApi(
                                'createObject: shape from string, initialProps: %o',
                                initialProps
                        );
                        ...
                        dApi(
                                'createObject: shape from object, initialProps: %o',
                                initialProps
                        );
                        if (!shape) {
                                log.error(
                        ...
                        log.error(
                                'VTT_API.createObject: Invalid arguments.',
                                arg1,
                                arg2
                        );
                        dApi(
                                'createObject error: Invalid arguments. %o, %o',
                                arg1,
                                arg2
                        );
                        return null; // Or throw error
        ...
        updateObject: (objectId, updatedProps) => {
                ...
                dApi(
                        'updateObject: model.updateObject returned: %o',
                        updatedObj
                );
                // requestRedrawEvent(); // Removed: model.updateObject should trigger 'modelChanged'
                return updatedObj;
        ...
        showMessage: (text, type, duration) => {
                ...
                if (typeof showMessageCallback === 'function') {
                        ...
                        log.warn(
                                `VTT_API.showMessage (fallback): ${type} - ${text}`
                        );
                        dApi(
                                'showMessage: showMessageCallback NOT available, fallback used.'
                        );
        ...
        setBoardProperties: (properties) => {
                ...
                const currentBoardProps =
                        model.updateBoardProperties(properties);
                dApi(
