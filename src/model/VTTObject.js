...
class VTTObject {
    ...
    if (initialProps) {
      for (const key in initialProps) {
        if (Object.hasOwn(initialProps, key)) {
          if (
            typeof initialProps[key] === "object" &&
            initialProps[key] !== null &&
            !Array.isArray(initialProps[key]) &&
            key in defaults &&
          ...
          } else if (key !== "id" && key !== "shape") {
            // id and shape are already handled
  ...
  update(updatedProps) {
    dVTTObject(
      "update called for id: %s, updatedProps: %o",
      this.id,
      updatedProps,
    );
    let changed = false;
    ...
    for (const key in updatedProps) {
      if (Object.hasOwn(updatedProps, key) && key !== "id") {
        ...
        if (
          ...
          changed = true;
          dVTTObject(
            "Property %s changed from %o to %o",
            key,
            oldValue,
            newValue,
          );
