...
const initializeApplication = async () => {
  ...
  const uiCallbacks = {
    ...
    onLoadFromFileInputChange: (event) => {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          d('File content loaded, calling sessionManagement.handleLoadTableState');
  ...
  document.addEventListener('modelChanged', (event) => {
    d('modelChanged event received in main.js: Type - %s, Payload - %o', event.detail.type, event.detail.payload);
      ...
      switch (type) {
        ...
        case 'objectDeleted':
          ...
          if (payload.id === VTT_API.getSelectedObjectId()) {
             canvasViewModel.setSelectedObjectInViewModel(null);
             d('CanvasViewModel: selected object %s was deleted, selection reset', payload.id);
          }
        ...
        default:
          d('Unhandled modelChanged event type in main.js for CanvasViewModel: %s', type);
      }
...
if (document.readyState === 'loading') {
  d('DOM not fully loaded, adding DOMContentLoaded listener for initializeApplication');
