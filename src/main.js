...
const initializeApplication = async () => {
                ...
                onLoadFromFileInputChange: (event) => {
                        ...
                        if (file) {
                                ...
                                reader.onload = (e) => {
                                        ...
                                        sessionManagement.handleLoadTableState(
                                                e.target.result
                                        );
                                };
                                reader.onerror = (e) => {
        ...
        document.addEventListener('modelChanged', (event) => {
                ...
                if (event.detail && canvasViewModel) {
                        ...
                        d(
                                'Processing modelChanged event for CanvasViewModel: Type - %s',
                                type
                        );
                        switch (type) {
                        ...
                        case 'allObjectsCleared':
                                ...
                                canvasViewModel.setSelectedObjectInViewModel(
                                        null
                                );
                                d(
                                        'CanvasViewModel: allObjectsCleared and selection reset'
                                );
                                break;
                        case 'selectionChanged':
                                ...
                                d(
                                        'CanvasViewModel: selectionChanged to %s',
                                        payload
                                );
                                break;
                        case 'objectAdded':
                                canvasViewModel.addObjectToViewModel(
                                        payload
                                );
                                d(
                                        'CanvasViewModel: objectAdded: %o',
                                        payload
                                );
                                break;
                        case 'objectUpdated':
                                canvasViewModel.updateObjectInViewModel(
                                        payload.id,
                                        payload
                                );
                                d(
                                        'CanvasViewModel: objectUpdated: %s, %o',
                                        payload.id,
                                        payload
                                );
                                break;
                        case 'objectDeleted':
                                canvasViewModel.removeObjectFromViewModel(
                                        payload.id
                                );
                                if (
                                        payload.id ===
                                                VTT_API.getSelectedObjectId()
                                ) {
                                        canvasViewModel.setSelectedObjectInViewModel(
                                                null
                                        );
                                ...
                                d(
                                        'CanvasViewModel: objectDeleted: %s',
                                        payload.id
                                );
                                break;
                        case 'panZoomChanged':
                                canvasViewModel.setPanZoomInViewModel(
                                        payload
                                );
                                d(
                                        'CanvasViewModel: panZoomChanged: %o',
                                        payload
                                );
                                break;
                        case 'backgroundChanged':
                                canvasViewModel.setBackgroundInViewModel(
                                        payload
                                );
                                d(
                                        'CanvasViewModel: backgroundChanged: %o',
                                        payload
                                );
                                break;
                        case 'boardPropertiesChanged':
                                canvasViewModel.setBoardPropertiesInViewModel(
                                        payload
                                );
                                d(
                                        'CanvasViewModel: boardPropertiesChanged: %o',
                                        payload
                                );
                                break;
                        ...
                        d(
                                'modelChanged event received, but canvasViewModel is not available.'
                        );
                }
                requestRedraw();
        });
        ...
        if (canvasViewModel) {
                ...
                d(
                        'Initial state for CanvasViewModel: %o',
        ...
        uiViewModel.displayMessage(
                'Application initialized (Offline Mode).',
                'info'
        );
