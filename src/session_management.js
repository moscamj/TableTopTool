...
const triggerDownload = (filename, data) => {
        ...
        dSession(
                'Download triggered and resources cleaned up for: %s',
                filename
        );
...
export const handleSaveTableState = () => {
        ...
        dSession(
                'Table state save process completed for filename: %s',
                filename
        );
};
...
export const handleLoadTableState = (fileContent) => {
        ...
        try {
                ...
                if (!loadedState || typeof loadedState !== 'object') {
                        dSession(
                                'Load error: Invalid file format or missing critical data.'
                        );
                        throw new Error(
                                'Invalid file format or missing critical data.'
                        );
                }
                const {
                ...
                if (!Array.isArray(loadedObjectsArray)) {
                        dSession(
                                'Load error: Invalid file format - objects is not an array.'
                        );
                        throw new Error(
                                'Invalid file format: objects is not an array.'
                        );
                }
                ...
                dSession(
                        'Loading %d objects from file.',
                        loadedObjectsArray.length
                );
                ...
                if (loadedViewState) {
                        dSession(
                                'Setting pan/zoom state from loaded state: %o',
                                loadedViewState
                        );
...
export const applyMemoryState = (stateObject) => {
        ...
        if (!stateObject || typeof stateObject.objects === 'undefined') {
                ...
                dSession(
                        'applyMemoryState error: Invalid memory state object provided.'
                );
                VTT_API.showMessage(
                        'Invalid memory state object provided.',
                        'error'
                );
                return false;
        }
        ...
        VTT_API.clearAllObjects();
        dSession(
                'Loading %d objects from memory state.',
                stateObject.objects.length
        );
...
export const getMemoryStateByIndex = (index) => {
        ...
        if (index >= 0 && index < inMemoryStates.length) {
                ...
                dSession(
                        'Returning memory state at index %d: %o',
                        index,
                        state
                );
                return state;
