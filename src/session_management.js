...
const triggerDownload = (filename, data) => {
  ...
  a.href = url;
  a.download = filename;
  dSession('Triggering download for filename: %s, data length: %d', filename, data.length);
...
export const handleSaveTableState = () => {
  dSession('handleSaveTableState called. Current session ID: %s', currentSessionId);
  const state = {
...
export const handleLoadTableState = (fileContent) => {
  dSession('handleLoadTableState called with fileContent length: %d', fileContent.length);
  try {
    ...
    if (loadedBackground) {
      dSession('Setting table background from loaded state: %o', loadedBackground);
    ...
    if (loadedBoardProperties) {
      dSession('Setting board properties from loaded state: %o', loadedBoardProperties);
      VTT_API.setBoardProperties(loadedBoardProperties);
    }

    if (loadedSessionId) {
      dSession('Updating currentSessionId from %s to %s', currentSessionId, loadedSessionId);
    ...
    dSession('Table state loaded successfully. Session ID: %s', currentSessionId);
  } catch (error) {
...
export const handleSaveMemoryState = () => {
  ...
  dSession('State pushed to inMemoryStates. Current count: %d', inMemoryStates.length);
  if (inMemoryStates.length > MAX_IN_MEMORY_STATES) { // Uses MAX_IN_MEMORY_STATES
    inMemoryStates.length = MAX_IN_MEMORY_STATES;
    dSession('Exceeded MAX_IN_MEMORY_STATES. Trimmed to %d states.', MAX_IN_MEMORY_STATES);
  }
...
export const applyMemoryState = (stateObject) => {
  ...
  if (stateObject.background) {
    dSession('Setting table background from memory state: %o', stateObject.background);
    VTT_API.setTableBackground(stateObject.background);
  }
  if (stateObject.viewState) {
    dSession('Setting pan/zoom state from memory state: %o', stateObject.viewState);
    VTT_API.setPanZoomState(stateObject.viewState);
  }
  if (stateObject.boardProperties) {
    dSession('Setting board properties from memory state: %o', stateObject.boardProperties);
  ...
  dSession('Memory state applied successfully: %s', stateObject.name || 'Unnamed State');
  return true;
};
...
export const getAvailableMemoryStates = () => {
  dSession('getAvailableMemoryStates called. Number of states: %d', inMemoryStates.length);
...
export const getMemoryStateByIndex = (index) => {
  dSession('getMemoryStateByIndex called with index: %d', index);
    ...
    dSession('getMemoryStateByIndex error: Invalid index %d. Max index: %d', index, inMemoryStates.length - 1);
    return null;
};
