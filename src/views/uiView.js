...
export const init = (vttApi) => {
  ...
  const uiCallbacks = {
    ...
    onLoadFromFileInputChange: (event) => {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          dUiView(
            "File content loaded in uiView, calling sessionManagement.handleLoadTableState",
          );
  ...
  if (domElements.vttCanvas) {
    ...
    log.error(
      "[uiView.js] VTT Canvas element not found, cannot initialize canvasView.",
    );
    uiViewModelInstance.displayMessage(
      "Critical Error: Canvas element not found. VTT will not function.",
      "error",
    );
  }
...
export const initUIEventListeners = (callbacks, vttApi) => {
  const { onSaveToFile, onLoadFromFileInputChange, onSaveMemoryState } =
    callbacks;
  dUiView(
    "initUIEventListeners called with callbacks: %o and vttApi: %o",
    callbacks,
    vttApi,
  );
